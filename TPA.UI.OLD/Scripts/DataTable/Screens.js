...
function renderdataTable() {
    $('#myDatatable').DataTable({
        ...
        "columns": [
            ...
            { "data": "APP_NAME", "name": "APP NAME", "autoWidth": true },
            {
                "data": "SEC_SCREEN_ID", "orderable": false, "width": "3%", "render": function (data, type, row) {

                    return '<button id="' + row.SEC_SCREEN_ID + '" data-id="' + data + '" onclick="editScreen(' + row.SEC_SCREEN_ID + ')" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="tooltip" data-placement="top"  title="Edit"><i class="la la-edit"></i></button>'
                        +
                        '<button id="' + row.SEC_SCREEN_ID + '" onclick="deleteEvent(this)" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="tooltip" data-placement="top"  title="Remove"><i class="fas fa-trash-alt "></i></button>'
                        ;
                }
            }
...
function deleteEvent(obj) {
    ...
    swal.fire({
        ...
        focusConfirm: false,
        preConfirm: (login) => {
            return fetch(deleteUri + "?screenId=" + rowID)
                .then(response => {
                    if (!response.ok) {
