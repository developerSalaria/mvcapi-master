        ...
        "columns": [
            ...
            { "data": "User_Name_ENG", "name": "User_Name_ENG", "autoWidth": true },
            {
                "data": "User_ID", "orderable": false, "width": "3%", "render": function (data, type, row) {
                    ...
                    return '<button id="' + row.User_ID + '" data-id="' + data + '" onclick="editUser(' + row.User_ID+')" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="tooltip" data-placement="top"  title="Edit"><i class="la la-edit"></i></button>'
                        +
                        '<button id="' + row.User_ID + '" onclick="deleteEvent(this)" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="tooltip" data-placement="top"  title="Remove"><i class="fas fa-trash-alt "></i></button>'
                        ;
                }
            }
...
function deleteEvent(obj) {
    ...
    swal.fire({
        ...
        focusConfirm: false,
        preConfirm: (login) => {
            return fetch(deleteUri + "?userId=" + rowID)
                .then(response => {
                    if (!response.ok) {
