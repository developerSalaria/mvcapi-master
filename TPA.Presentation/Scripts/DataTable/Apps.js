...
function renderdataTable() {
    $('#myDatatable').DataTable({
        ...
        "columns": [
            ...
            {
                "data": "SEC_APP_ID", "orderable": false, "width": "3%", "render": function (data, type, row) {

                    return '<button id="' + row.SEC_APP_ID + '" data-id="' + data + '" onclick="editApp(' + row.SEC_APP_ID + ')" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="tooltip" data-placement="top"  title="Edit"><i class="la la-edit"></i></button>'
                        +
                        '<button id="' + row.SEC_APP_ID + '" onclick="deleteEvent(this)" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="tooltip" data-placement="top"  title="Remove"><i class="fas fa-trash-alt "></i></button>'
                        ;
                }
            }
...
function deleteEvent(obj) {
    ...
    swal.fire({
        ...
        focusConfirm: false,
        preConfirm: (login) => {
            return fetch(deleteUri + "?appId=" + rowID)
                .then(response => {
                    if (!response.ok) {
